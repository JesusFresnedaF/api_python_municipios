    ...
    <body>
        ...
        <script>
            function realizarAccion(ruta) {{
                fetch(`http://{server_host}:800${{ruta}}`)
            ...
            function filtrar() {{    
                const municipio = document.getElementById('municipio').value || "palma";
                realizarAccion(`/get/${{encodeURIComponent(municipio)}}`);
            }}

            function mostrarTotal() {{
                realizarAccion('/Total_alojamientos');
            }}
    </script>
    </body>
